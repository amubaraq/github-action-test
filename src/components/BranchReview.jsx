...
// import { Send, Star, X, User } from "lucide-react";
// import Modal from "react-modal";

// // Bind modal to app root for accessibility
// Modal.setAppElement("#root");

// const BranchReview = ({ branchSlug }) => {
...
//     try {
//       const response = await fetch(`${backendURL}/api/branch/${branchSlug}`, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       const data = await response.json();
//       if (data.status === "success" && data.branch) {
//         setReviews(data.branch?.details?.reviews || []);
//       } else {
//         setErrorReviews("No reviews found for this branch");
//       }
//     } catch (err) {
//       setErrorReviews("Error fetching branch reviews");
//     } finally {
...
//       try {
//         const response = await fetch(`${backendURL}/api/review/branch`, {
//           method: "POST",
//           headers: {
//             "Content-Type": "application/json",
//             Authorization: `Bearer ${token}`,
//           },
//           body: JSON.stringify({ branch_slug: branchSlug, ...values }),
//         });
//         const result = await response.json();
//         if (!response.ok) {
//           throw new Error(result.message || "Failed to submit review");
//         }
//         showAlert("Branch review submitted successfully!", "success");
...
//           <span className="text-blue-500">★</span> Branch Reviews
//         </h2>
//         {userInfo && token && (
//           <button
...
//                   <div className="flex">
//                     <div
//                       className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-medium ${review.avatar_url ? "" : getAvatarColor(review.reviewer_name)}`}>
//                       {review.avatar_url ? (
//                         <img
...
//           },
//         }}
//         contentLabel="Branch Review Modal">
//         <div className="flex justify-between items-center mb-4">
//           <h2 className="text-lg font-semibold text-gray-800">
//             Submit a Branch Review
//           </h2>
//           <button
...
const BranchReview = ({ branchSlug }) => {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.status === "success" && data.branch) {
        // Enrich reviews with a fallback reviewerName
        const enrichedReviews = (data.branch?.details?.reviews || []).map(
          (review) => ({
            ...review,
  ...
  const getInitials = (name) => {
    if (!name) return "A"; // Default to "A" for Anonymous
    return name.charAt(0).toUpperCase();
  };

  const getAvatarColor = (name) => {
    if (!name) return "bg-gray-500"; // Default color for Anonymous
    const colors = [
      "bg-blue-500",
      "bg-green-500",
      "bg-purple-500",
        ...
        <div>
          ...
          <div
            ...
            {reviews.map((review) => (
              <div
                key={review.id}
                className="review-card snap-start flex-shrink-0 w-auto max-w-[20rem] px-2">
                <div className="bg-gray-50 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                  <div className="flex flex-col items-center gap-4 p-4 bg-white shadow-md rounded-lg w-full max-w-lg mx-auto">
                    {/* Avatar */}
                    <div
                      className={`w-9 h-9 rounded-full flex items-center justify-center text-white font-medium text-lg ${getAvatarColor(
                        review.reviewerName
                      )}`}>
                      {review.avatar_url ? (
                        <img
                          ...
                          className="w-full h-full object-cover rounded-full"
                          onError={(e) =>
                            (e.target.src = `https://via.placeholder.com/40/${getAvatarColor(
                              review.reviewerName
                            ).replace("bg-", "")}/FFFFFF?text=${getInitials(
                              review.reviewerName
                            )}`)
                          }
                        />
