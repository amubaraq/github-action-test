...
const ReviewSection = ({ userSlug }) => {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${backendURL}/api/profile/${userSlug}`, {
  ...
  const handleSubmit = useCallback(
    async (values, { resetForm }) => {
      ...
      try {
        const response = await fetch(`${backendURL}/api/review/user`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
          body: JSON.stringify({ slug: userSlug, ...values }),
        });
    ...
    <div className="bg-white p-6 rounded-xl shadow-md" data-aos="fade-up">
      ...
      {loadingReviews ? (
        ...
        <div>
          ...
          {/* Custom Side-Scrolling Carousel */}
          <div
            ref={carouselRef}
